...
```bash
git clone https://github.com/your-username/meshy-mcp-server.git
cd meshy-mcp-server
npm install
npm run build
```
...
- **Network errors** — Transient failures (429, 500, 502, 503, 504) are retried automatically with exponential backoff (1s, 2s, 4s — max 3 retries)
...
meshy-mcp-server/
├── src/
│   ├── index.ts           # MCP server setup, 25 tool definitions, validation
